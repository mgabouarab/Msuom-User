...
extension HomeTypeCell: UICollectionViewDataSource {
...
extension HomeTypeCell: UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let item = self.items[indexPath.row]
        let vc = BrowseCarVC.create(type: .category(id: item.id, title: item.name))
        self.parentContainerViewController?.show(vc, sender: self)
    }
}
extension HomeTypeCell: UICollectionViewDelegateFlowLayout {
